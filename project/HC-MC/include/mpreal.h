...
#if defined(__MPFR_H) && \
    !(defined(MPFR_USE_NO_MACRO) && defined(MPFR_USE_INTMAX_T))
...
#define MPFR_USE_NO_MACRO  // Avoid name clash with MPFR, introduced in
                           // MPFR 4.2.0
#endif
#include <mpfr.h>
#endif
...
inline const mpreal frexp(const mpreal &x, mpfr_exp_t *exp,
                          mp_rnd_t mode = mpreal::get_default_rnd()) {
  mpreal y(x);
#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
...
inline mpreal minval(mp_prec_t prec) {
  // The smallest positive value in MPFR is 1/2 * 2^emin = 2^(emin - 1). However
  // it gives infinity if inverted. Overall safe minimum is 2^(emin + 1).
...
inline const mpreal root(const mpreal &x, const mpreal &n,
                         mp_rnd_t r = mpreal::get_default_rnd()) {
  if (isint(n) && mpfr_sgn(n.mpfr_ptr()) > 0)
    return root(x, n.toULong(), r);
  else {
    mpreal y(0, (std::max)(mpfr_get_prec(x.mpfr_srcptr()),
                           mpfr_get_prec(n.mpfr_srcptr())));
    ...
    if (isnan(x) || isnan(n))
      ...
      mpfr_setsign(y.mpfr_ptr(), y.mpfr_srcptr(), mpfr_signbit(x.mpfr_srcptr()),
                   r);
    }
...
inline void sincos(const mpreal &x, mpreal *sin, mpreal *cos,
                   mp_rnd_t rnd_mode = mpreal::get_default_rnd()) {
...
/* f(x) = f(pi*x) */
inline const mpreal cospi(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(cospi);
}
inline const mpreal sinpi(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(sinpi);
}
inline const mpreal tanpi(const mpreal &x,
                          mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(tanpi);
}
inline const mpreal acospi(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(acospi);
}
inline const mpreal asinpi(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(asinpi);
}
inline const mpreal atanpi(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(atanpi);
}

inline const mpreal log2p1(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log2p1);
} /* log2 (1+x) */
inline const mpreal log10p1(const mpreal &x,
                            mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(log10p1);
} /* log10(1+x) */
inline const mpreal exp2m1(const mpreal &x,
                           mp_rnd_t r = mpreal::get_default_rnd()) {
  MPREAL_UNARY_MATH_FUNCTION_BODY(exp2m1);
} /* 2^x-1      */
inline const mpreal exp10m1(const mpreal &x,
                            mp_rnd_t r = mpreal::get_default_rnd()) {
...
inline bool mpreal::fits_in_bits(double x, int n) {
  int i;
  double t;
  return MPREAL_IS_INF(x) ||
         (std::modf(std::ldexp(std::frexp(x, &i), n), &t) == 0.0);
}
